...
describe('formatItemPost / formatClosingReply', () => {
  it('combines headline + commentary with no link', () => {
    const item = { headline: 'Headline here', commentary: 'Commentary text.' };
...
describe('publishThreadsChain', () => {
  ...
  it('publishes the full 4-post chain and returns all 4 ids in order', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(jsonResponse({ id: 'c1' })).mockResolvedValueOnce(jsonResponse({ id: 'p1' })) // lead
      .mockResolvedValueOnce(jsonResponse({ id: 'c2' })).mockResolvedValueOnce(jsonResponse({ id: 'p2' })) // reply1
      .mockResolvedValueOnce(jsonResponse({ id: 'c3' })).mockResolvedValueOnce(jsonResponse({ id: 'p3' })) // reply2
      .mockResolvedValueOnce(jsonResponse({ id: 'c4' })).mockResolvedValueOnce(jsonResponse({ id: 'p4' })); // closing
    vi.stubGlobal('fetch', fetchMock);

    const result = await publishThreadsChain(digest, config);
    ...
    const reply1CreateBody = JSON.parse(fetchMock.mock.calls[2][1].body);
    expect(reply1CreateBody.reply_to_id).toBe('p1');
    const reply2CreateBody = JSON.parse(fetchMock.mock.calls[4][1].body);
    expect(reply2CreateBody.reply_to_id).toBe('p2');
    const closingCreateBody = JSON.parse(fetchMock.mock.calls[6][1].body);
  ...
  it('stops at the first failure with no orphaned replies attempted', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(jsonResponse({ id: 'c1' })).mockResolvedValueOnce(jsonResponse({ id: 'p1' })) // lead ok
    ...
    // Only 3 calls happened: lead create+publish, then reply1's failed create.
    // reply1's publish and reply2/closing never fire — no orphaned replies.
