...
// everywhere: HTTPS only, well-formed, no ASCII control characters, bounded
// length. It returns the WHATWG-normalized `href` — the only form that should
// ever be stored or handed to the fetcher.
//
// No source domain allowlist: any well-formed HTTPS URL is accepted. Junk and
// paywalled sources are the submitter's problem to manage, not this
// validator's — it only enforces that the URL itself is well-formed and safe
// to store/fetch. SSRF protection against private/internal addresses is
// enforced separately, at fetch time, in safe-fetch.js (DNS resolution +
// private-IP rejection on every redirect hop), independent of this allowlist
// having existed or not.
//
...
//
// Zero dependencies (only the global WHATWG URL) so scripts/local-fetcher.js
// can import it directly without pulling in the server's module graph.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Validate and normalize an article URL. No domain restriction — format and
 * safety only (well-formed, HTTPS, no control characters, bounded length).
 * @param {unknown} raw
