...
- **Facebook publishing is NOT actually configured yet.** `FACEBOOK_PAGE_ID` and `FACEBOOK_PAGE_ACCESS_TOKEN` are both empty in the server `.env` — confirmed 2026-07-22 while diagnosing the "Unexpected non-whitespace character after JSON" publish failure. The page setup (`docs/facebook-page-setup.md`, now in English, page is facebook.com/theghostbrief) was never completed. Telegram publishing works; Facebook will keep failing (now with a readable error) until real credentials are set.
